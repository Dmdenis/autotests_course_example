...
class RomanNums:
    ...
    def from_roman(self):
        ...
        result = 0
        for index, value in enumerate(self.roman):
            if index + 1 < len(self.roman) and integers[self.roman[index]] < integers[self.roman[index + 1]]:
                result -= integers[self.roman[index]]
            else:
                result += integers[self.roman[index]]
        return result

    def is_palindrome(self):
        """определяет, является ли арабское число палиндромом (True - является, иначе False)"""
